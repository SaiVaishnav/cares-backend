  ...
  try {
    if (req.body.profileImage !== undefined) {
      ...
      if (user)
        return res
          .status(401)
          .json({ message: "Email pehle se register ho rakha hai." });
      ...
      if (user)
        return res
          .status(401)
          .json({ message: "There already exists a user with this username" });
      console.log(username, password, firstName, lastName);
      const newUser = new User({
        role: "basic",
        emailId,
...
exports.googleSignin = async (req, res) => {
  ...
  try {
    ...
    client
      .verifyIdToken({ idToken: tokenId, audience: process.env.CLIENTID })
      .then(async (response) => {
        const {
          email_verified,
          name,
...
exports.login = async (req, res) => {
  try {
    ...
    if (!user)
      return res
        .status(401)
        .json({ message: "yeh " + emailId + " galat email hai" });
    if (!user.comparePassword(password))
      return res.status(401).json({ message: "galat hai email/pass" });
    if (!user.isVerified)
      return res
        .status(401)
        .json({ type: "not-verified", message: "Verify nai hua." });
...
exports.verify = async (req, res) => {
  ...
  try {
    ...
    if (!token) return res.status(400).json({ message: "link expire ho gaya" });
    User.findOne({ _id: token.userId }, (err, user) => {
      if (!user) return res.status(400).json({ message: "aisa koi nai hai" });
      if (user.isVerified)
...
exports.resendToken = async (req, res) => {
  try {
    ...
    if (!user)
      return res.status(401).json({
        message: "yeh " + req.body.email + " email register nai hua hai",
      });
    if (user.isVerified)
